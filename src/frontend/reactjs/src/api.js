...
const api = {
  hotel: {
    ...
    search: (params) =>
      callApi("GET", "hotel/search", params).then((resp) => resp.data),
    // 624429 <- from the search method
    getInfo: (hotelId) =>
      callApi("GET", `hotel/${hotelId}`).then((resp) => resp.data),
  },
  account: {
    // {
    //   name: 'bob',
    ...
    // }
    create: (body) =>
      callApi("POST", "account/", {}, body).then((resp) => resp.data),
    getAll: () => callApi("GET", "account/").then((resp) => resp.data),
    get: (accountId) =>
      callApi("GET", `account/${accountId}`).then((resp) => resp.data),
    // same as create but with accountId and body is all optional, can also be used for create
    update: (accountId, body) =>
      callApi("PATCH", `account/${accountId}`, {}, body).then(
        (resp) => resp.data
      ),
    delete: (accountId) =>
      callApi("DELETE", `account/${accountId}`).then((resp) => resp.data),
  },
  reservation: {
    // {
    ...
    // }
    create: (body) =>
      callApi("POST", "reservation/", {}, body).then((resp) => resp.data),
    // jerry@hotmail.com
    getAll: (accountId) =>
      callApi("GET", `reservation/${accountId}`).then((resp) => resp.data),
    // d07d6878-be86-4d69-961b-b9cbb741f96c <- from create or get methods
    delete: (reservationId) =>
      callApi("DELETE", `reservation/${reservationId}`).then(
        (resp) => resp.data
      ),
  },
  payment: {
    ...
    // }
    pay: (body) =>
      callApi("POST", "payment/pay", {}, body).then((resp) => resp.data),
  },
};
module.exports = api;
